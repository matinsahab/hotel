...
require('admintop.php');
	...
	$pos = $_POST['pos'];
	if ($pass !== $cpass) {
		header('location: aaccount.php?match');
		return $z;
	}
	elseif (!isset($z)) {
	$command1 = "UPDATE `user-account` SET `username` = '$user', `password` = md5('$pass'), `user-level` = 'Admin' 
				 WHERE `username` = '$user'";
	$query1 = mysqli_query($con,$command1);
	if ($query1) {
      header('location: admin.php?aaccount=Admin Password Changed');
  	}
	}
}
?>
 ...
 <h1 style="color: red; margin-bottom: 30px;">Update My Account</h1>
  <div class="form-row">
    <div class="form-group col-md-4"> 
      ...
      <input type="text" class="form-control" name="user" value="<?php echo $_SESSION['sadmin']; ?>" id="" readonly>
    </div>
    <div class="form-group col-md-4">
    ...
    <div class="form-group col-md-4">
      ...
      <input type="text" class="form-control" value="Admin" name="pos" readonly required>
